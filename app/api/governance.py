...
def activate_proposal(
    ...
    if not proposal:
        raise HTTPException(
            status_code=404, detail="Proposal not found or not eligible for activation"
        )
    return proposal
...
def list_proposals(
    ...
    """
    return governance_service.list_proposals(status=status, limit=limit, offset=offset)
