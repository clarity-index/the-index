...
class ClaimBase(BaseModel):
    ...
    semantic_representation: Optional[Dict[str, Any]] = Field(
        None, 
        description="Machine-readable semantic structure"
    )
...
class LinkBase(BaseModel):
    ...
    claim_id_2: Optional[str] = Field(None, description="ID of second claim (for claim-claim links)")
...
class EpistemicStatus(BaseModel):
    ...
    independence_score: float = Field(
        ...
        ge=0.0, 
        le=1.0,
        description="Diversity of evidence sources"
    )
    robustness_score: float = Field(
        default=0.0,
        ge=0.0,
        le=1.0,
        description="Replication and methodological quality"
    )
